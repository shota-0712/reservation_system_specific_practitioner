...
この図は主要なスキーマ領域をドメインごとに俯瞰し、どのテーブルがどのクラスタに属し、どのように横断するかを示す。
...
クラスタは実装側でも使われる論理領域 `Tenant/Auth`, `Catalog/Staff`, `Booking`, `CRM`, `Integrations`, `Exports/Notifications` を反映し、各ドメインがどのマスター/トランザクションを抱えるかを示している。
