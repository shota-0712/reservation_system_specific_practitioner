Chapter 12
Chapter 11: バックエンドセキュリティ実践 - OWASP Top 10対策とセキュア設計
GK
GK
2026.01.23に更新
このチャプターの目次
...
想定されるセキュリティ対策
Helmet導入 - セキュアヘッダー設定
CSRF対策 - すべての変更系エンドポイントに適用
レート制限 - 認証エンドポイントに5回/15分の制限
2要素認証 - 管理者とオプトインユーザーに導入
Prisma使用 - SQL Injectionを完全防止
定期的な脆弱性スキャン - GitHub Actionsで自動化
まとめ
セキュリティの成功の鍵
多層防御 - 単一の対策に頼らない
最小権限 - 必要最小限の権限のみ付与
継続的監視 - ログ、アラート、定期スキャン
開発者教育 - セキュアコーディングの徹底
定期的更新 - 依存関係、パッチの適用
次のステップ
今すぐ実装: Helmet + bcrypt + JWT
入力検証: Zodスキーマ検証
レート制限: express-rate-limit
監視: ログ + アラート
定期監査: npm audit + ペネトレーションテスト
参考資料
OWASP Top 10
