...
JWTのベストプラクティス
短い有効期限: アクセストークンは15分程度
リフレッシュトークン: 長期間有効なトークンで更新
HTTPS必須: トークンは必ずHTTPS経由で送信
強力なシークレット: 十分に長く複雑なシークレットキーを使用
センシティブ情報を含めない: パスワードなどはペイロードに含めない
// ❌ 悪い例
...
セキュリティインシデント	3件/月
実施した改善
MFA導入 - 全ユーザーに2段階認証を推奨
強力なパスワードポリシー - 最低12文字、複雑性要件
アカウントロックアウト - 5回失敗で15分ロック
不正検知 - 異常な位置・デバイスからのログインを検知
JWTの短命化 - アクセストークンを5分に短縮
改善後の結果
指標	値	改善率
不正ログイン試行	80件/日	-93.3%
アカウント乗っ取り	0件/月	-100%
...
認証と認可の違いを理解する
JWTを正しく実装する
リフレッシュトークンで安全性と利便性を両立
OAuth 2.0で外部サービス連携
RBACで柔軟なアクセス制御
MFAでセキュリティを強化
セキュリティベストプラクティスを常に実践
次のステップ
